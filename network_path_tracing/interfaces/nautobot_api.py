...
class NautobotAPISession:
    ...
    def get_json(self, path: str, **kwargs) -> Dict[str, Any]:
        """Return the response JSON for the supplied API path."""
...
class NautobotAPIDataSource(NautobotDataSource):
    ...
    def get_most_specific_prefix(self, address: str) -> Optional[PrefixRecord]:
        # Using a larger limit to ensure we can pick the most specific prefix.
    ...
    def _ensure_prefix_id(self, prefix: PrefixRecord) -> str:
        ...
        # Fallback: look up prefix by CIDR to retrieve its UUID; fall back to raw prefix
    ...
    def _expand_ip_record(self, record: dict) -> dict:
        """Fetch a depth-1 representation of an IP address to expose related objects."""
        ...
        try:
            expanded = self._session.get_json(
                f"/api/ipam/ip-addresses/{record_id}/", params={"depth": 1}
            )
    ...
    def _build_ip_record(
        self, record: dict, override_address: Optional[str] = None
    ) -> IPAddressRecord:
        """Convert a Nautobot IPAddress payload into an ``IPAddressRecord``."""
    ...
    def _resolve_assignment_details(self, record: dict) -> tuple[Optional[str], Optional[str]]:
        """Determine device and interface names associated with the IP."""
    ...
    def _fetch_device_name_via_api(self, record: dict) -> Optional[str]:
        ...
        # Virtual interfaces reference virtual machines instead of devices.
    ...
    def _resolve_names_from_interfaces(self, interfaces: list[dict]) -> tuple[Optional[str], Optional[str]]:
        """Follow interface references to discover owning device and interface names."""
    ...
    def _resolve_names_from_url(self, url: str) -> tuple[Optional[str], Optional[str]]:
        """Resolve device and interface names from an absolute API URL."""
    ...
    def _lookup_device_by_primary_ip(self, record: dict) -> Optional[str]:
        ...
        # Try device lookups first (IPv4/IPv6), then virtual machines.
        ip_id = record.get("id")
        search_patterns = [
            ("/api/dcim/devices/", {"primary_ip4_id": ip_id}),
            ("/api/dcim/devices/", {"primary_ip6_id": ip_id}),
            ("/api/dcim/devices/", {"primary_ip4": ip}),
            ("/api/dcim/devices/", {"primary_ip6": ip}),
            ("/api/dcim/devices/", {"primary_ip": ip}),
            ("/api/virtualization/virtual-machines/", {"primary_ip4_id": ip_id}),
            ("/api/virtualization/virtual-machines/", {"primary_ip6_id": ip_id}),
    ...
    def _extract_name_from_relationship(related: Any) -> Optional[str]:
        """Return a readable name from a related object reference."""
    ...
    def _extract_names_from_payload(self, payload: dict) -> tuple[Optional[str], Optional[str]]:
        """Extract device and interface names from a Nautobot object payload."""
